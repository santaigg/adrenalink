...
import { fetchTournament, TournamentData, MatchStats, PlayerStats } from "@/app/utils/fetch/fetchTournament";
...
export default function TournamentPage() {
  ...
  const currentMatch = tournamentData?.matches && selectedMatch
  ...
  const allPlayers = currentMatch ? [
    ...(currentMatch.team1?.players?.map(player => ({ ...player, team: currentMatch.team1.name })) || []),
    ...(currentMatch.team2?.players?.map(player => ({ ...player, team: currentMatch.team2.name })) || [])
  ] : [];
  ...
  if (error || !tournamentData) {
    return (
      <main className="h-full flex flex-col items-center justify-center">
        <div className="text-xl text-red-500">{error || "Failed to load tournament data"}</div>
        <button 
  ...
  return (
    <main className="h-full flex flex-col">
        ...
        <Constrict className={`px-4 ${isMaximized ? 'max-w-none !w-full' : ''}`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-primary-foreground">{tournamentData.name}</h1>
                <div className="flex items-center space-x-2">
                  <span className="animate-pulse text-accent text-xs">●</span>
                  <span className="text-accent text-xs font-medium">LIVE NOW</span>
                </div>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              >
                <svg
                  className={`w-3 h-3 ${refreshing || willAutoRefresh ? 'animate-spin' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <span>{refreshing ? 'Refreshing...' : willAutoRefresh ? 'Auto-refreshing...' : 'Refresh'}</span>
              </button>
              <button 
                ...
                >
                  <div className="flex flex-col w-40">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="truncate max-w-[80px]">{match.team1.name}</span>
                      <div className="flex items-center space-x-1">
                        <span className="font-bold">{match.team1.score}</span>
                        <span>-</span>
                        <span className="font-bold">{match.team2.score}</span>
                      </div>
                      <span className="truncate max-w-[80px] text-right">{match.team2.name}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="flex flex-col xl:flex-row gap-4">
            ...
            <div className="w-full xl:w-3/4 bg-secondary/70 border border-primary-foreground/5 rounded-primary corner-clip-sm overflow-hidden">
              ...
              <div className="p-3 flex justify-between items-center">
                <div className="flex items-center">
                  <span className="text-base md:text-lg font-medium mr-2">{currentMatch?.team1.name}</span>
                  <span className="text-lg md:text-xl font-bold">{currentMatch?.team1.score}</span>
                  <span className="mx-2 text-secondary-foreground">vs</span>
                  <span className="text-lg md:text-xl font-bold">{currentMatch?.team2.score}</span>
                  <span className="text-base md:text-lg font-medium ml-2">{currentMatch?.team2.name}</span>
                </div>
            ...
            <div className="w-full xl:w-1/4 bg-secondary/70 border border-primary-foreground/5 rounded-primary corner-clip-sm overflow-hidden">
              <div className="p-3 border-b border-secondary">
                <h2 className="text-lg font-bold text-primary-foreground">Tournament Stats</h2>
              </div>
              
              <Tabs defaultValue="overview" onValueChange={(value) => setActiveTab(value as "overview" | "players")}>
                  ...
                  <div className="p-3">
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <h3 className="text-base font-medium">{currentMatch?.round}</h3>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className={`bg-secondary/50 p-2 rounded-primary corner-clip-sm ${
                          currentMatch?.status === "completed" && currentMatch?.team1.score > currentMatch?.team2.score
                            ? "border-2 border-accent/50" 
                            : ""
                        }`}>
                          <div className="flex justify-between items-center">
                            <span className="font-medium">{currentMatch?.team1.name}</span>
                            <span className="text-lg font-bold">{currentMatch?.team1.score}</span>
                          </div>
                              ...
                              >
                                <div className="flex items-center">
                                  <div className="mr-1 h-4 w-4">
                                    <SponsorImage sponsor={player.sponsor} className="h-4" />
                                  </div>
                                  <span>@{player.handle}</span>
                                </div>
                                <span className="text-secondary-foreground">{player.kills} K / {player.deaths} D</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className={`bg-secondary/50 p-2 rounded-primary corner-clip-sm ${
                          currentMatch?.status === "completed" && currentMatch?.team2.score > currentMatch?.team1.score
                            ? "border-2 border-accent/50" 
                            : ""
                        }`}>
                          <div className="flex justify-between items-center">
                            <span className="font-medium">{currentMatch?.team2.name}</span>
                            <span className="text-lg font-bold">{currentMatch?.team2.score}</span>
                          </div>
                          <div className="mt-2 space-y-1">
                      ...
                      {currentMatch && currentMatch.map && mapImages[currentMatch.map] && (
                        <div className="mt-3">
                          <h4 className="text-xs font-medium mb-2">Match Map</h4>
                          <div className="relative aspect-video rounded-primary corner-clip-sm overflow-hidden">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-2">
                              <p className="text-sm font-medium text-primary-foreground">{currentMatch.map}</p>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {currentMatch?.status === "completed" && (
                        <div className="mt-3">
                          <h4 className="text-xs font-medium mb-1">Match Statistics</h4>
                          <div className="grid grid-cols-1 gap-2">
                            <div className="bg-secondary/30 p-2 rounded-primary corner-clip-sm">
                              <div className="text-xs text-secondary-foreground">Rounds Won</div>
                              <div className="text-sm">{currentMatch.team1.score} - {currentMatch.team2.score}</div>
                            </div>
                          </div>
                        </div>
                      )}
                ...
                <TabsContent value="players" className="p-3">
                  <h3 className="text-sm font-medium mb-2">Player Stats - {currentMatch?.round}</h3>
                        ...
                        <div className="flex-grow min-w-0">
                          <div className="flex justify-between items-center">
                            <div className="truncate">
                              ...
                              <span className="text-xs text-secondary-foreground ml-1">({player.team})</span>
                            </div>
                            <span className="text-accent text-sm font-medium">{player.kd.toFixed(2)} K/D</span>
                          </div>
      ...
      <div className="h-full bg-primary w-full">
        ...
        <Constrict className={`px-4 py-8 ${isMaximized ? 'max-w-none !w-full' : ''}`}>
          {activeTab === "overview" ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-secondary/70 border border-primary-foreground/5 rounded-primary corner-clip-sm p-3">
                <h3 className="text-base font-bold mb-3">Tournament Schedule</h3>
                <div className="space-y-2">
                  {tournamentData.matches.map((match, index) => (
                    <div key={index} className="flex justify-between items-center py-1 border-b border-secondary/50">
              ...
              <div className="bg-secondary/70 border border-primary-foreground/5 rounded-primary corner-clip-sm p-3">
                ...
                <div className="space-y-1 text-sm">
                  <p><span className="text-secondary-foreground">Format:</span> Single Elimination</p>
                  <p><span className="text-secondary-foreground">Prize Pool:</span> $10,000</p>
                  <p><span className="text-secondary-foreground">Teams:</span> 3</p>
                  <p><span className="text-secondary-foreground">Hosted by:</span> Optic Gaming</p>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {sortedPlayers.slice(0, 3).map((player, index) => (
                    ...
                    >
                      ...
                      <span className="text-accent">{player.kd.toFixed(2)} K/D</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-secondary/70 border border-primary-foreground/5 rounded-primary corner-clip-sm p-4">
              <h3 className="text-lg font-bold mb-4">Detailed Player Statistics</h3>
                      ...
                      >
                        ...
                        <td className="py-2 px-2">
                          <div className="flex items-center">
                            ...
                            <span className="text-primary-foreground hover:underline">@{player.handle}</span>
                          </div>
                        </td>
                        <td className="py-2 px-2">{player.team}</td>
                        <td className="py-2 px-2">
                          <div className="flex items-center">
                            <SponsorImage sponsor={player.sponsor} className="h-4 mr-1" />
                            <span>{player.sponsor}</span>
                          </div>
                        </td>
                        <td className="py-2 px-2 text-right">{player.kills}</td>
                        <td className="py-2 px-2 text-right">{player.deaths}</td>
                        <td className="py-2 px-2 text-right">{player.assists}</td>
                        <td className="py-2 px-2 text-right text-accent font-medium">{player.kd.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
